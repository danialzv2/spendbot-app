...
export async function verifyPin(pin) {
  const res = await fetch(`${API_URL}/auth`, { method: 'POST', headers: { 'Authorization': `Bearer ${pin}` } })
  return res.ok
}
...
export async function uploadReceipt(file, chatId) {
  ...
  const res = await fetch(`${API_URL}/receipt?chat_id=${chatId}`, { method: 'POST', headers: authHeaders(), body: formData })
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  if (!res.ok) throw new Error(`API error ${res.status}`)
  return res.json()
}
...
export async function updateTransaction(chatId, timestamp, { amount, category, place, note }) {
  const res = await fetch(`${API_URL}/transaction`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
    body: JSON.stringify({ chat_id: chatId, timestamp, amount, category, place, note }),
  })
  if (res.status === 401) throw new Error('UNAUTHORIZED')
...
export async function deleteTransaction(chatId, timestamp) {
  ...
  const res = await fetch(`${API_URL}/transaction?${params}`, { method: 'DELETE', headers: authHeaders() })
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  if (res.status === 404) throw new Error('NOT_FOUND')
  if (!res.ok) throw new Error(`API error ${res.status}`)
  return res.json()
...
export async function fetchDashboard(chatId) {
  const res = await fetch(`${API_URL}/dashboard?chat_id=${chatId}`, { headers: authHeaders() })
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  if (!res.ok) throw new Error(`API error ${res.status}`)
  return res.json()
}
...
export async function getConfig(chatId) {
  const res = await fetch(`${API_URL}/config?chat_id=${chatId}`, { headers: authHeaders() })
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  if (!res.ok) throw new Error(`API error ${res.status}`)
  return res.json()
}
