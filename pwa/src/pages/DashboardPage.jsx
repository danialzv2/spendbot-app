...
// ── Combined Pace + Daily Safe Spend ──────────────────────────────────────────

function MetricCell({ label, sublabel, value, valueColor }) {
  return (
    <div style={{
      background: 'rgba(255,255,255,0.03)', borderRadius: 10,
      padding: '0.65rem 0.7rem', border: '1px solid rgba(255,255,255,0.05)',
    }}>
      <div style={{ fontSize: '0.6rem', fontWeight: 500, color: 'var(--text-muted)', lineHeight: 1.4, marginBottom: '0.4rem' }}>
        {label}
        {sublabel && <div style={{ color: 'var(--text-muted)', opacity: 0.75, marginTop: '0.1rem', fontSize: '0.57rem' }}>{sublabel}</div>}
      </div>
      <div style={{ fontFamily: 'var(--mono)', fontSize: '0.9rem', fontWeight: 700, color: valueColor || 'var(--text-primary)', letterSpacing: '-0.3px' }}>
        {value}
      </div>
    </div>
  )
  ...
  return (
    <div className="glass-panel" style={{ padding: '1rem 1.1rem' }}>
      ...
      <div className="pace-track">
        <div className="pace-fill" style={{ width: `${spendPct}%`, background: spendPct > 90 ? 'var(--negative)' : spendPct > 70 ? 'var(--warning)' : 'var(--positive)' }} />
      </div>
      <div style={{ fontFamily: 'var(--mono)', fontSize: '0.58rem', color: 'var(--text-muted)', marginTop: '0.35rem', textAlign: 'right' }}>
        {spendPct.toFixed(0)}% of disposable
      </div>
    </div>
  )
}
...
function SpendingHeatmap({ heatmap }) {
  ...
  if (total === 0) return (
    <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.82rem', padding: '1.5rem 0' }}>No spending this month yet</div>
  )
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '0.4rem', marginBottom: '0.4rem' }}>
        {DOW_SHORT.map(d => (
          <div key={d} style={{ fontFamily: 'var(--mono)', fontSize: '0.58rem', color: 'var(--text-muted)', textAlign: 'center', textTransform: 'uppercase' }}>{d}</div>
        ))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '0.4rem', marginBottom: '0.5rem' }}>
        {values.map((val, i) => (
          <div key={i} title={`${DOW[i]}: RM ${val.toFixed(2)}`} style={{
            height: 36, borderRadius: 8,
            background: val === 0 ? 'rgba(255,255,255,0.03)' : `rgba(200, 245, 100, ${0.1 + (val / maxVal) * 0.75})`,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '0.4rem' }}>
        {values.map((val, i) => (
          <div key={i} style={{ fontFamily: 'var(--mono)', fontSize: '0.56rem', color: val > 0 ? 'var(--text-secondary)' : 'var(--text-muted)', textAlign: 'center' }}>
            {val > 0 ? `RM${val.toFixed(0)}` : '—'}
          </div>
        ))}
      </div>
    </div>
...
function AnomalyCard({ anomaly }) {
  return (
    <div style={{
      ...
      borderRadius: 12, padding: '0.55rem 0.85rem', marginBottom: '0.4rem',
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: '0.5rem', marginBottom: '0.18rem' }}>
        ...
        <div style={{ fontFamily: 'var(--mono)', fontSize: '0.58rem', color: 'var(--text-muted)', flexShrink: 0, whiteSpace: 'nowrap' }}>
          {anomaly.timestamp?.slice(0, 16)}
        </div>
      </div>
...
function TxRow({ tx, chatId, onDeleted, onUnauth }) {
  ...
  return (
    <div className="tx-row" style={{ background: isConfirm ? 'rgba(248,113,113,0.06)' : undefined, opacity: isDeleting ? 0.4 : 1, transition: 'all 0.2s ease' }}>
      ...
      <div className="tx-amount" style={{ marginRight: '0.5rem' }}>RM {parseFloat(tx.amount).toFixed(2)}</div>
      <button
        ...
        style={{
          ...
          borderRadius: 7, padding: isConfirm ? '0.3rem 0.5rem' : '0.35rem',
          cursor: isDeleting ? 'default' : 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          flexShrink: 0, minWidth: isConfirm ? 52 : 30, transition: 'all 0.15s ease',
        }}
      >
        {isDeleting ? (
...
function ManualKeyIn({ chatId, onAdded, onUnauth }) {
  ...
  async function handleSubmit() {
    ...
    setFeedback(null)
    try {
      const result = await addTransaction(chatId, {
        amount: amt, category,
  ...
  return (
    <div className="glass-panel" style={{ padding: '1rem 1.1rem' }}>
      ...
      <div style={{ marginBottom: '0.6rem' }}>
        <div style={{ fontSize: '0.62rem', fontWeight: 600, color: '#c0c0c0', marginBottom: '0.35rem' }}>
          Place <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(optional)</span>
        </div>
        <input
          value={place}
          onChange={e => setPlace(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && canSubmit && handleSubmit()}
          placeholder="e.g. McDonald's, Grab, Watsons"
          style={fieldStyle}
        />
      </div>
      ...
      {feedback && (
        <div style={{
          marginTop: '0.65rem', padding: '0.5rem 0.75rem', borderRadius: 8,
          background: feedback.ok ? 'rgba(200,245,100,0.08)' : 'rgba(248,113,113,0.08)',
          border: `1px solid ${feedback.ok ? 'rgba(200,245,100,0.2)' : 'rgba(248,113,113,0.2)'}`,
          fontFamily: 'var(--mono)', fontSize: '0.72rem',
          color: feedback.ok ? 'var(--accent)' : 'var(--negative)',
...
export default function DashboardPage({ chatId, onUnauth }) {
  ...
  function handleDeleted(timestamp) {
    setData(prev => {
      ...
      return {
        ...prev,
        months:   prev.months.map(m => ({ ...m, transactions: rm(m.transactions) })),
        all_time: { ...prev.all_time, transactions: rm(prev.all_time.transactions) },
      }
    })
  }

  // ── Optimistic add, then refresh ───────────────────────────────────────────
  function handleTransactionAdded(newTx) {
    ...
    // Refresh from server after short delay to sync totals and categories
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', animation: 'fadeIn 0.2s ease' }}>
      ...
      <div style={{ padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '1.6rem', paddingBottom: '3rem' }}>
        ...
        {insights?.pace && (
          <div>
            <SectionHead>Spending Pace</SectionHead>
            <SpendingPaceSection pace={insights.pace} dailySafe={insights.daily_safe_spend} />
          </div>
        )}

        {/* ── Savings Projection ── */}
        {insights?.savings?.has_config && (
          <div>
            <SectionHead>Savings This Month</SectionHead>
            <SavingsProjection data={insights.savings} />
          </div>
        )}

        {/* ── Spending Heatmap ── */}
        <div>
          <SectionHead>Spending by Day · This Month</SectionHead>
          <div className="glass-panel" style={{ padding: '1rem 1.1rem' }}>
            <SpendingHeatmap heatmap={insights?.heatmap || {}} />
          </div>
        </div>
        ...
        {allAnomalies.length > 0 && (
          <div>
            <SectionHead>
              Unusual Activity
              <span style={{ float: 'right', color: 'var(--negative)', fontWeight: 400 }}>{allAnomalies.length} flagged</span>
            </SectionHead>
            {visibleAnomalies.map((a, i) => <AnomalyCard key={i} anomaly={a} />)}
            {!showAllAnomalies && hiddenCount > 0 && (
              <button className="btn-icon" onClick={() => setShowAllAnomalies(true)} style={{ width: '100%', textAlign: 'center', marginTop: '0.25rem', fontSize: '0.72rem', padding: '0.5rem' }}>
                Show More ({hiddenCount} more)
              </button>
            )}
            {showAllAnomalies && allAnomalies.length > 3 && (
              <button className="btn-icon" onClick={() => setShowAllAnomalies(false)} style={{ width: '100%', textAlign: 'center', marginTop: '0.25rem', fontSize: '0.72rem', padding: '0.5rem' }}>
                Show Less ↑
              </button>
            )}
          </div>
        ...
        <div>
          <SectionHead>
            ...
            <span style={{ float: 'right', color: 'var(--text-secondary)', fontWeight: 400 }}>{allTransactions.length} total</span>
          </SectionHead>
